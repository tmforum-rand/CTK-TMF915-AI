...
var pmCollection = require('./CTK-AI_Management-4.0.0.postman_collection.json');
...
Object.keys(config['payloads']).forEach(resource => {
    ...
    if (resource == "AiModel") {
        pmCollection['item'][15]['item'][0]['request']['body']['raw'] = JSON.stringify(config['payloads'][resource]['POST']['payload'])
    }
    if (resource == "Rule") {
        pmCollection['item'][18]['item'][0]['request']['body']['raw'] = JSON.stringify(config['payloads'][resource]['POST']['payload'])
    }
